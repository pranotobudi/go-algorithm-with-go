...
func Factor(primes []int, number int) []int {
	...
	var dividend = number
	// var lastIdx int
	for !Contains(primes, dividend) {
		for _, prime := range primes {
			// fmt.Println("dividend: ", dividend, " prime: ", prime)
			if dividend%prime == 0 {
		...
		// fmt.Println("outerloop dividend: ", dividend)
	}
	//add last dividend
	res = append(res, dividend)
	return res
}

func Contains(primes []int, number int) bool {
	for _, prime := range primes {
		if prime == number {
			return true
		}
